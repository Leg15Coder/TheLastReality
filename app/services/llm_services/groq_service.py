...
from app.services.llm_services.base_llm_service import BaseLLMService, ModelTypes, ModelUnavailableError, AnswerSize
...
class GroqService(BaseLLMService):
    def __init__(self) -> None:
        ...
        with open(rankings_file, 'r', encoding='utf-8') as f:
            ...
            self.models = rankings.get('groq', None)
            if self.models is None:
                logger.warning("Модели Groq не подгружены")
                self.api_key = None
    ...
    def save_response_to_logs(self, model: str, response: Response) -> None:
        if self.settings.debug:
            try:
                ...
                file_path = logs_dir / f"groq_{response.status_code}_{model}_{ts}.json"
    ...
    def _post_generate_content_with_retries(self, model: str, parts: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                raise ModelUnavailableError(f"Groq {model} слишком много запросов (429). Модель временно приостановлена.")

            if response.status_code == 404:
                self.unavailable_models[model] = time.time() + 1 + random.random() * 1000
                raise ModelUnavailableError(f"Groq {model} не найден (404). Модель временно приостановлена.")

            if 500 <= response.status_code < 600:
                logger.warning(f"Groq 5xx ошибка: {response.status_code}")
                time.sleep(1 + random.random())

                raise RuntimeError(f"Ошибка Groq API {response.status_code}: {response.text}")

        except requests.RequestException as e:
            logger.warning(f"Ошибка сети в сервисе Groq: {e}")
            time.sleep(1 + random.random())

        raise RuntimeError("Groq исчерпал все попытки запроса.")
    ...
    async def generate_text(
        ...
        if not result:
            raise RuntimeError(f"Groq не вернула ответ ни на один запрос; last_error={last_error}")
        ...
        if response_mime_type == "application/json":
            ...
            try:
                ...
                parsed_json = None
                logger.warning("Не удалось распарсить JSON из ответа groq: %s", ex)
