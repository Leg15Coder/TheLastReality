...
class ZhipuService(BaseLLMService):
    def __init__(self) -> None:
        super().__init__()
        self.api_key = self.settings.zhipu_api_key
        self.timeout = 30
        self.max_retries = 5

        self.service_name = "zhipu"
        ...
        with open(rankings_file, 'r', encoding='utf-8') as f:
            ...
            self.models = rankings.get(self.service_name, None)
            if self.models is None:
                logger.warning(f"Модели {self.service_name.title()} не подгружены")
                self.api_key = None
